...
<?php
  ...
  if ($sortOption == 'Rating') {
    // Sort by Rating 
    $sql_statement = "SELECT pseudoname, review, rating FROM tangled_garden_reviews ORDER BY CAST(SUBSTRING_INDEX(rating, ' ', 1) AS DECIMAL) DESC";
  } else {
    // Sort by Recent 
    $sql_statement = "SELECT pseudoname, review, rating FROM tangled_garden_reviews ORDER BY added DESC";
  }
...
//format data for database submission
    ...
    if (empty($reviewerName) || empty($reviewerEmail) || empty($reviewText) || empty($rating)) {
        ...
        if (!in_array($rating, $valid_ratings)) {
            ...
            $sql_statement = "INSERT INTO tangled_garden_reviews (pseudoname, email, review, rating) 
...
<html>
  ...
  <body class="d-flex flex-column min-vh-100">
    ...
    <!-- Main Title -->
    <h3 style="text-align: center;">Tangled Garden</h3> 
