...
<?php
  ...
  if ($sortOption == 'Rating') {
    // Sort by Rating 
    $sql_statement = "SELECT pseudoname, review, rating FROM the_real_scoop_reviews ORDER BY CAST(SUBSTRING_INDEX(rating, ' ', 1) AS DECIMAL) DESC";
  } else {
    // Sort by Recent 
    $sql_statement = "SELECT pseudoname, review, rating FROM the_real_scoop_reviews ORDER BY added DESC";
  }
...
//format data for database submission
    ...
    if (empty($reviewerName) || empty($reviewerEmail) || empty($reviewText) || empty($rating)) {
        ...
        if (!in_array($rating, $valid_ratings)) {
            ...
            $sql_statement = "INSERT INTO the_real_scoop_reviews (pseudoname, email, review, rating) 
...
<html lang="en">
  ...
  <body class="d-flex flex-column min-vh-100">
    ...
    <!-- Main Title -->
    <h3 style="text-align: center;">The Real Scoop</h3>
    
    <!-- Reviews Slideshow Section -->
    <section class="container my-4">
      <h3 style="text-align: center;">Customer Reviews</h3>
      <div class="scroll-container">
        <!-- Existing hard-coded review cards -->
        <div class="card mx-2" style="min-width: 300px;">
          <div class="card-body">
            <h5 class="card-title">John Doe</h5>
            <p class="card-text">"Amazing experience! The ambiance and service were top-notch."</p>
        ...
        <div class="card mx-2" style="min-width: 300px;">
          <div class="card-body">
            ...
            <p class="card-text">"Great food, though a bit pricey. I’d definitely come back!"</p>
            <p class="card-text"><small>Rating: 4 ★</small></p>
          </div>
        </div>
        <div class="card mx-2" style="min-width: 300px;">
          <div class="card-body">
            <h5 class="card-title">Alex Johnson</h5>
            <p class="card-text">"The dessert selection was incredible. Loved every bite!"</p>
            <p class="card-text"><small>Rating: 4.5 ★</small></p>
          </div>
        </div>
        <!-- Additional reviews added via submit_review.html will appear below -->
      </div>
    </section>
    ...
    <!--OPENS A LIST OF ALL REVIEW FOR THIS PLACE-->
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content" style="color: black;">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="myModalLabel" >The Real Scoop Reviews</h5>
  ...
  <div class="modal-dialog modal-lg" role="document">
    ...
    <!-- Script to load stored reviews from localStorage -->
     <!--
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        var reviewContainer = document.querySelector(".scroll-container");
        var storedReviews = localStorage.getItem("reviews");
        if (storedReviews) {
          var reviews = JSON.parse(storedReviews);
          reviews.forEach(function (review) {
            // Create a new card element for each review
            ...
            textPara.className = "card-text";
            textPara.textContent = '"' + review.text + '"';

            var ratingPara = document.createElement("p");
            ratingPara.className = "card-text";
            var small = document.createElement("small");
            small.textContent = "Rating: " + review.rating + " ★";
            ratingPara.appendChild(small);
